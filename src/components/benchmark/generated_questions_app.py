...
with st.form("generate_questions_form"):
    no_question = st.number_input("Number of questions to generate", min_value=1, max_value=20)
    ...
    if submitted:
        if file_response.type_message == TypeMessage.WARNING:
            ...
            if generated_question_resp.type_message == TypeMessage.ERROR or generated_question_resp.type_message == TypeMessage.ERROR:
                get_alert_message(generated_question_resp.type_message, generated_question_resp.message)  
        else:
            get_alert_message(file_response.type_message, file_response.message)
