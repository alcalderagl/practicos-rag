...
def chuncking_expander(
    ...
    with st.expander("Chunking Document"):
        ...
        for i, page in enumerate(
            pagesCleaned, start=1
        ):  # `start=1` for human-readable page numbers
            st.write(f"Processing Page {i}...")

            # Call the `chuncking_doc` function with the correct page number
            chuncks: ResponseLogic = chuncking_doc(
                page, file_name, page_number=i, output_file=output_file
            )
